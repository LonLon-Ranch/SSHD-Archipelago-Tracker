            ...
            {!activeSubmap && (
                <>
                    ...
                    {mapModel.provinces.map((submap) => {
                        ...
                        // Skip provinces that don't have map data (e.g., 'sky', 'silentRealms', 'specialAreas')
                        if (!entry || !('markerX' in entry)) {
                            return null;
                        }
                    ...
                    {/* Also display regions from provinces without submaps (sky, silentRealms, specialAreas) */}
                    {mapModel.provinces
                        .filter((p) => !mapData[p.provinceId] || !('markerX' in mapData[p.provinceId]))
                        .flatMap((p) => p.regions)
                        .map((marker) => (
                            <div key={marker.hintRegion}>
                                <MapMarker
